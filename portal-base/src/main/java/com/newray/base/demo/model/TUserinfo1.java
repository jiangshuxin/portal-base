...
//Demo
@Entity
@Table(name = "T_USERINFO1")
@DataGrid(width=1100,height=350,sortName="id",sortOrder="desc")
@TableGenerator(name="NewrayTG",table="TABLE_PK_GENERATOR", pkColumnName="pk_name",pkColumnValue="TUserinfo.id",valueColumnName="pk_value", allocationSize=1)
public class TUserinfo1 implements Serializable {
	...
	public TUserinfo1(BigDecimal id) {
		this.id = id;
	}

	public TUserinfo1(BigDecimal id, String usercode, String password,
	...
	@Id
	@GeneratedValue(strategy=GenerationType.TABLE,generator="NewrayTG")
	@ColumnUI(frozen=true,title="编号",width=200,sortable=true,order="00100")
	@QueryCondition(order="00100")
	@InfoUI(id="ID", title="编号", order="00100", isRequired=true, validClass="easyui-validatebox")
	...
	@InfoUI(id="PASSWORD", title="密码",order="00300", isRequired=true)
	...
	@InfoUI(id="USERNAME", title="用户名",order="00200", isRequired=true)
	...
	@InfoUI(id="ADDRESS", title="地址", order="00400")
